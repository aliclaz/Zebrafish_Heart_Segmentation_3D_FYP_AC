...
def show_history(history, model, backbone, out_path):
    ...
    ax[0,0].title('Training and Validation Loss for {} with {} backbone'.format(model, backbone))
    ...
    ax[0,1].title('Training and Validation IOU for {} with {} backbone'.format(model, backbone))
...
def show_all_historys(historys, models, backbones, out_path):
    ...
    for i in range(len(historys)):
        ...
        ax[i,0].title('Training and Validation Loss for {} with {} backbone'.format(models[i], backbones[i]))
        ...
        ax[i,1].title('Training and Validation IOU for {} with {} backbone'.format(models[i], backbones[i]))
...
def show_val_masks(models, imgs, gts, preds, out_path):
    ...
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            if j == 0:
                ...
                ax[i,j].set_title('Predicted Mask by {}'.format(models[j - 2]))
                ax[i,j].imshow(preds[j-2,slices[k],:,:,:,0], cmap='gray')
    plt.savefig(out_path+'val_imgs_and_masks.jpg')
    plt.show()

def show_test_masks(models, imgs, preds, out_path):
    ...
    for i in range(n_rows):
        ...
        for j in range(n_cols):
            if j == 0:
                ...
                ax[i,j].set_title('Predicted Mask by {}'.format(models[j - 1]))
